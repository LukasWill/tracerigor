...
def get_image(card_name):
    """Render a small card face without relying on third-party image assets."""
    if card_name in image_cache:
        return image_cache[card_name]

    image = Image.new("RGB", (70, 98), "white")
    draw = ImageDraw.Draw(image)
    font = ImageFont.load_default()
    if card_name == "card":
        draw.rounded_rectangle((2, 2, 67, 95), radius=5, fill="#244a73", outline="white", width=2)
        draw.line((10, 10, 60, 88), fill="#8bb6dc", width=2)
        draw.line((60, 10, 10, 88), fill="#8bb6dc", width=2)
    else:
        suit_code, face = card_name[0], card_name[1:]
        suit = {"C": "♣", "D": "♦", "H": "♥", "S": "♠"}.get(suit_code, suit_code)
        color = "#b31b1b" if suit_code in {"D", "H"} else "#111111"
        draw.rounded_rectangle((1, 1, 68, 96), radius=5, outline="#222222", width=2)
        draw.text((7, 7), f"{face}{suit}", fill=color, font=font)
        draw.text((35, 49), suit, fill=color, font=font, anchor="mm")
...
def is_natural(hand):  # Is this hand a natural blackjack?
    return sorted(card[0] if isinstance(card, tuple) else card for card in hand) == [1, 10]
    ...
    * v0: Initial version release
    ...
    def _get_obs(self):
        if self.is_pixel:
            ...
            player_values = [card[0] for card in self.player]
            return (sum_hand(self.player), self.dealer[0][0], usable_ace(player_values))
