...
class BaseEnvConfig(ABC):
    ...
    # Standard special tokens including ReflAct framework's <reflection> tags
    special_token_list: Optional[List[str]] = field(
        default_factory=lambda: [
            "<think>", "</think>",
            "<action>", "</action>",
            "<answer>", "</answer>",  # legacy, kept for backward compat
            "<reflection>", "</reflection>"  # ReflAct framework (arXiv:2505.15182)
        ]
    )
    action_sep: str = ","
    @abstractmethod
