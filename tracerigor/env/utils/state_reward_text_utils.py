...
def _run_llm_judge(inputs):
    """Import the optional judge stack only when process rewards are enabled."""
    from tracerigor.server.llm_as_judge import run_llm_judge

    return run_llm_judge(inputs)


def _run_llm_judge_new(inputs):
    """Import the optional structured judge only when it is requested."""
    from tracerigor.server.llm_as_judge_sokoban_frozenlake import run_llm_judge
...
def service_state_reward_wrapper(step_batch_func):
    def wrapped_step_batch(self, ids2actions):
        ...
        if len(input_to_llm) > 0:
            ...
            results = _run_llm_judge(input_to_llm)
        else:
            return step_batch_results
...
def service_state_reward_wrapper_v2(step_batch_func):
    def wrapped_step_batch(self, ids2actions):
        ...
        if len(input_to_llm) > 0:
            # Use synchronous batch processing
            results = _run_llm_judge_new(input_to_llm) # a dict containing a set of metrics
        else:
            return step_batch_results
