...
def _process_batch(prompts: List[Union[str, List[Dict[str, str]]]], config) -> List[Dict[str, Any]]:
                ...
                while retries <= config.get("max_retries", 3):
                    try:
                        ...
                        async with rate_limiter.semaphore:
                            per_req_timeout = config.get("per_request_timeout", 45)
    ...
    try:
        ...
        if loop and loop.is_running():
            # If you ever call this from an already-running loop (e.g. FastAPI),
            # offload the coroutine to a dedicated thread and block for the result.
            import concurrent.futures
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as ex:
                fut = ex.submit(asyncio.run, _async_batch_completions())
                return fut.result()
        else:
            return asyncio.run(_async_batch_completions())
    except Exception as e:
